...
void raster_to_texture(rasterlist_t *r)
{
	...
	if (old_list) glstate->list.active = nullptr;		// deactivate list...
...
void bitmap_flush() {
	...
	if (glstate->raster.bm_tnwidth < glstate->raster.bm_width || glstate->raster.bm_tnheight < glstate->raster.bm_height) {
		...
		gl4es_glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, glstate->raster.bm_tnwidth, glstate->raster.bm_tnheight, 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
	}
