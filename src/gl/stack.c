...
void APIENTRY_GL4ES gl4es_glPushAttrib(GLbitfield mask) {
    ...
    cur->mask = mask;
    cur->clip_planes_enabled = nullptr;
    cur->clip_planes = nullptr;
    cur->lights_enabled = nullptr;
    cur->lights = nullptr;
    cur->materials = nullptr;
...
void APIENTRY_GL4ES gl4es_glPushClientAttrib(GLbitfield mask) {
    ...
    noerrorShim();
    if (glstate->clientStack == nullptr) {
...
DBG(SHUT_LOGD("glPopAttrib()\n");)
    ...
    if (glstate->stack == nullptr || glstate->stack->len == 0) {
...
void APIENTRY_GL4ES gl4es_glPopClientAttrib(void) {
    ...
    if (glstate->clientStack == nullptr || glstate->clientStack->len == 0) {
        errorShim(GL_STACK_UNDERFLOW);
        return;
    }
