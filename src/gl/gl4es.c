...
void APIENTRY_GL4ES gl4es_glEnd(void) {
    ...
    if(glstate->list.compiling) {
        ...
        if (!globals4es.beginend /*|| (glstate->polygon_mode==GL_LINE)*/) {
            ...
            withColor = (mylist->color!=nullptr);
            glstate->list.active = nullptr;
            mylist = end_renderlist(mylist);
            draw_renderlist(mylist);
            free_renderlist(mylist);
        } else {
            withColor = (glstate->list.active->color!=nullptr);
...
void UnBuffer()
{
    for (int i=0; i<NB_VA; i++)
        if(glstate->vao->locked_mapped[i]) {
            ...
            glstate->vao->vertexattrib[i].real_pointer = nullptr;
...
GLuint APIENTRY_GL4ES gl4es_glGenLists(GLsizei range) {
    ...
    for (int i = 1; i <= range; i++) {
        ...
        if (k == kh_end(lists)){
            ...
            kh_value(lists, k) = nullptr;  // create an empty gllist
        }
    }
    return start + 1;
}
...
void gl4es_flush() {
    if(glstate->list.compiling)
        return;
    // flush internal list
    renderlist_t *mylist = glstate->list.active?extend_renderlist(glstate->list.active):nullptr;
    if (mylist) {
        glstate->list.active = nullptr;
        glstate->list.pending = 0;
        mylist = end_renderlist(mylist);
...
#endif
        bindBuffer(GL_ARRAY_BUFFER, glstate->scratch_vertex);
        gles_glBufferData(GL_ARRAY_BUFFER, alloc, nullptr, GL_STREAM_DRAW);
...
void gl4es_scratch_indices(int alloc) {
    ...
    if(glstate->scratch_indices_size < alloc) {
        gles_glBufferData(GL_ELEMENT_ARRAY_BUFFER, alloc, nullptr, GL_DYNAMIC_DRAW);
