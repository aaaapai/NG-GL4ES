...
void set_uniforms_default_value(GLuint program, uniforms_declarations uniformVector, int uniformCount) {
    for (int i = 0; i < uniformCount; i++) {
        ...
        if (strstr(uniform->initial_value, "mat4") != nullptr) {
        ...
        else if (strstr(uniform->initial_value, "mat3") != nullptr) {
            GLfloat matValues[9];
        ...
        else if (strstr(uniform->initial_value, "mat2") != nullptr) {
            GLfloat matValues[4];
        ...
        else if (strstr(uniform->initial_value, "vec4") != nullptr) {
            GLfloat vecValues[4];
        ...
        else if (strstr(uniform->initial_value, "vec3") != nullptr) {
            // 处理 vec3 类型
            GLfloat vecValues[3];
        ...
        else if (strstr(uniform->initial_value, "vec2") != nullptr) {
            GLfloat vecValues[2];
        ...
        else if (strstr(uniform->initial_value, "float") != nullptr) {
            GLfloat value = strtof(uniform->initial_value, nullptr);
            gl4es_glUniform1f(location, value);
        }
        else if (strstr(uniform->initial_value, "int") != nullptr) {
            GLint value = strtol(uniform->initial_value, nullptr, 10);
            gl4es_glUniform1i(location, value);
        }
        else if (strstr(uniform->initial_value, "bool") != nullptr) {
        ...
        else if (strstr(uniform->initial_value, "sampler2D") != nullptr) {
...
char* FindAndCorrect(char* source, int* length, int mode) {
   const char*     template = mode == MODE_SWITCH ? switch_template : mode == MODE_CASE ? case_template : nullptr;
...
char * BackportConstArrays(char *source, int * sourceLength){
    ...
    if( tokenStart != nullptr && (tokenStart - source) < variableNameStart){
