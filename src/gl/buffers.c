...
void APIENTRY_GL4ES gl4es_glBindBufferBase(GLenum target, GLuint index, GLuint buffer) {
    ...
    if (buffer == 0) {
        ...
        glstate->vao->vertexattrib[index].real_pointer = nullptr;
    }
    else {
        // Search for the existing buffer or create a new one if not found
        khint_t k;
        int ret;
        glbuffer_t* buff = nullptr;
        khash_t(buff)* list = glstate->buffers;
        k = kh_get(buff, list, buffer);
        if (k == kh_end(list)) {
        ...
        glstate->vao->vertexattrib[index].real_pointer = nullptr; // Real buffer pointer can be updated later
...
void APIENTRY_GL4ES gl4es_glBufferData(GLenum target, GLsizeiptr size, const GLvoid * data, GLenum usage) {
    ...
    if (buff==nullptr) {
        ...
        LOGE("Warning, nullptr buffer for target=0x%04X for glBufferData\n", target);
        return;
    }
    if(target==GL_ARRAY_BUFFER)
...
void APIENTRY_GL4ES gl4es_glBufferSubData(GLenum target, GLintptr offset, GLsizeiptr size, const GLvoid * data) {
    ...
    if (buff==nullptr) {
		errorShim(GL_INVALID_OPERATION);
        DBG(SHUT_LOGD("LIBGL: Warning, nullptr buffer for target=0x%04X for glBufferSubData\n", target);)
        return;
    }
            ...
            if (t) {    // don't allow to remove default one
                ...
                if (k != kh_end(list)) {
                    ...
                    if (glstate->vao->vertex == buff)
                        glstate->vao->vertex = nullptr;
                    if (glstate->vao->elements == buff)
                        glstate->vao->elements = nullptr;
                    ...
                    for (int j = 0; j < hardext.maxvattrib; j++)
                        if (glstate->vao->vertexattrib[j].buffer == buff) {
                            glstate->vao->vertexattrib[j].buffer = nullptr;
...
void* APIENTRY_GL4ES gl4es_glMapBufferRange(GLenum target, GLintptr offset, GLsizeiptr length, GLbitfield access)
{
	...
	if (buff==nullptr) {
        errorShim(GL_INVALID_VALUE);
		return nullptr;		// Should generate an error!
    }
    if(buff->mapped) {
        errorShim(GL_INVALID_OPERATION);
        return nullptr;
...
VISIBLE void glBufferStorage(GLenum target, GLsizeiptr size, const void* data, GLbitfield flags) {
    ...
    if (data) {
        ...
        bufferObj->original_data = nullptr;
    }
...
void VaoSharedClear(glvao_t *vao) {
    if(vao==nullptr || vao->shared_arrays==nullptr)
        return;
    ...
    vao->vert.ptr = nullptr;
    vao->color.ptr = nullptr;
    vao->secondary.ptr = nullptr;
    vao->normal.ptr = nullptr;
    for (int i=0; i<hardext.maxtex; i++)
        vao->tex[i].ptr = nullptr;
    vao->shared_arrays = nullptr;
}
