...
GLboolean is_list_compatible(renderlist_t* list) {
    #define T2(AA, A, B) \
    if(glstate->vao->AA!=(list->B!=nullptr)) return GL_FALSE;
    #define TEST(A,B) T2(vertexattrib[A].enabled, A, B)
    #define TESTA(A,B,I) T2(vertexattrib[A+i].enabled, A+i, B[i])
...
renderlist_t *arrays_to_renderlist(renderlist_t *list, GLenum mode,
    ...
    if(glstate->vao->shared_arrays) {
        #define OP(A, N) (A)?(A+skip*N):nullptr
...
void APIENTRY_GL4ES gl4es_glDrawRangeElements(GLenum mode, GLuint start, GLuint end, GLsizei count, GLenum type, const void *indices) {
	...
	noerrorShim();
    GLushort *sindices = nullptr;
    GLuint *iindices = nullptr;
    bool need_free = !(
    ...
    if(need_free) {
        sindices = copy_gl_array((glstate->vao->elements)?(void*)((char*)glstate->vao->elements->data + (uintptr_t)indices):indices,
            type, 1, 0, GL_UNSIGNED_SHORT, 1, 0, count, nullptr);
    } else {
...
void APIENTRY_GL4ES gl4es_glDrawElements(GLenum mode, GLsizei count, GLenum type, const GLvoid *indices) {
    DBG(SHUT_LOGD("glDrawElements(%s, %d, %s, %p), vtx=%p map=%p, pending=%d\n", PrintEnum(mode), count, PrintEnum(type), indices, (glstate->vao->vertex)?glstate->vao->vertex->data:nullptr, (glstate->vao->elements)?glstate->vao->elements->data:nullptr, glstate->list.pending);)
...
void APIENTRY_GL4ES gl4es_glDrawArrays(GLenum mode, GLint first, GLsizei count) {
    ...
    if (glstate->list.active) {
        ...
        if(globals4es.mergelist && list->stage>=STAGE_DRAW && is_list_compatible(list) && !list->use_glstate) {
            list = NewDrawStage(list, mode);
            if(list->vert) {
                glstate->list.active = arrays_add_renderlist(list, mode, first, count+first, nullptr, 0);
                NewStage(glstate->list.active, STAGE_POSTDRAW);
                return;
            }
        }
    ...
    if (intercept) {
        renderlist_t *list;
        list = arrays_to_renderlist(nullptr, mode, first, count+first);
        list = end_renderlist(list);
        draw_renderlist(list);
        free_renderlist(list);
    } else {
        if (mode==GL_QUADS) {
            ...
            glDrawElementsCommon(GL_TRIANGLES, 0, count*3/2, count, indices+(first-indfirst)*3/2, nullptr, 1);
            wantBufferIndex(old_buffer);
            return;
        }

        glDrawElementsCommon(mode, first, count, count, nullptr, nullptr, 1);
    }
}
    ...
    for (int i=0; i<primcount; i++) {
        ...
        if (compiling) {
            if(globals4es.mergelist && glstate->list.active->stage>=STAGE_DRAW && is_list_compatible(glstate->list.active) && !glstate->list.active->use_glstate) {
                ...
                glstate->list.active = arrays_add_renderlist(glstate->list.active, mode, first, count+first, nullptr, 0);
                NewStage(glstate->list.active, STAGE_POSTDRAW);
                continue;
            }
        ...
        if (intercept) {
            ...
            if(globals4es.mergelist && list->stage>=STAGE_DRAW && is_list_compatible(list) && !list->use_glstate) {
                list = NewDrawStage(list, mode);
                list = arrays_add_renderlist(list, mode, first, count+first, nullptr, 0);
...
void APIENTRY_GL4ES gl4es_glDrawElementsBaseVertex(GLenum mode, GLsizei count, GLenum type, const void *indices, GLint basevertex) {
    DBG(SHUT_LOGD("glDrawElementsBaseVertex(%s, %d, %s, %p, %d), vtx=%p map=%p, pending=%d\n", PrintEnum(mode), count, PrintEnum(type), indices, basevertex, (glstate->vao->vertex)?glstate->vao->vertex->data:nullptr, (glstate->vao->elements)?glstate->vao->elements->data:nullptr, glstate->list.pending);)
    ...
    if(basevertex==0) {
        ...
        if(type==GL_UNSIGNED_INT && hardext.elementuint && !compiling && !intercept)
            iindices = copy_gl_array((glstate->vao->elements)?(void*)((char*)glstate->vao->elements->data + (uintptr_t)indices):indices,
                type, 1, 0, GL_UNSIGNED_INT, 1, 0, count, nullptr);
        else
            sindices = copy_gl_array((glstate->vao->elements)?(void*)((char*)glstate->vao->elements->data + (uintptr_t)indices):indices,
                type, 1, 0, GL_UNSIGNED_SHORT, 1, 0, count, nullptr);
...
void APIENTRY_GL4ES gl4es_glDrawArraysInstanced(GLenum mode, GLint first, GLsizei count, GLsizei primcount) {
    ...
    if (intercept) {
        ...
        if (mode==GL_QUADS) {
            ...
            GLuint old_buffer = wantBufferIndex(0);
            glDrawElementsCommon(GL_TRIANGLES, 0, count*3/2, count, indices+(first-indfirst)*3/2, nullptr, primcount);
            wantBufferIndex(old_buffer);
            return;
        }

        glDrawElementsCommon(mode, first, count, count, nullptr, nullptr, primcount);
    }
}
...
void APIENTRY_GL4ES gl4es_glDrawElementsInstancedBaseVertex(GLenum mode, GLsizei count, GLenum type, const void *indices, GLsizei primcount, GLint basevertex) {
    DBG(SHUT_LOGD("glDrawElementsInstanceBaseVertex(%s, %d, %s, %p, %d, %d), vtx=%p map=%p, pending=%d\n", PrintEnum(mode), count, PrintEnum(type), indices, primcount, basevertex, (glstate->vao->vertex)?glstate->vao->vertex->data:nullptr, (glstate->vao->elements)?glstate->vao->elements->data:nullptr, glstate->list.pending);)
    if(basevertex==0)
