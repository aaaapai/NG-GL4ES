...
GLint APIENTRY_GL4ES gl4es_glRenderMode(GLenum mode) {
	...
	if (mode == GL_SELECT) {
		if (glstate->selectbuf.buffer == nullptr)	{// error, cannot use Select Mode without select buffer
...
void select_glDrawArrays(const vertexattrib_t* vtx, GLenum mode, GLuint first, GLuint count) {
	if (count == 0) return;
	if (vtx->pointer == nullptr) return;
	if (glstate->selectbuf.buffer == nullptr) return;
	GLfloat *vert = copy_gl_array(vtx->pointer, vtx->type, 
			vtx->size, vtx->stride,
			GL_FLOAT, 4, 0, count+first, nullptr);
...
void select_glDrawElements(const vertexattrib_t* vtx, GLenum mode, GLuint count, GLenum type, GLvoid * indices) {
	...
	GLushort *sind = (GLushort*)((type==GL_UNSIGNED_SHORT)?indices:nullptr);
	GLuint *iind = (GLuint*)((type==GL_UNSIGNED_INT)?indices:nullptr);
