...
int list2VBO(renderlist_t* list)
{
    ...
    gles_glBufferData(GL_ARRAY_BUFFER, vbo_base, nullptr, GL_STATIC_DRAW);
    for(int i=0; i<imax; ++i) {
        array2vbo_t *r = work+sorted[i];
...
void listActiveVBO(renderlist_t* list, save_vbo_t* saved) {
    if(list->vert) {
        ...
        glstate->vao->vertexattrib[ATT_VERTEX].buffer = nullptr;
    }
    if(list->color) {
        ...
        glstate->vao->vertexattrib[ATT_COLOR].buffer = nullptr;
    }
    if(list->secondary) {
        ...
        glstate->vao->vertexattrib[ATT_SECONDARY].buffer = nullptr;
    }
    if(list->fogcoord) {
        ...
        glstate->vao->vertexattrib[ATT_FOGCOORD].buffer = nullptr;
    }
    if(list->normal) {
        ...
        glstate->vao->vertexattrib[ATT_NORMAL].buffer = nullptr;
    }
    for (int a=0; a<list->maxtex; ++a) {
        if(list->tex[a]) {
            ...
            glstate->vao->vertexattrib[ATT_MULTITEXCOORD0+a].buffer = nullptr;
        }
    }
}
...
//printf("colors=%f, %f, %f, %f / %f, %f, %f, %f\n", list->color[0],list->color[1],list->color[2],list->color[3], list->color[4],list->color[5],list->color[6],list->color[7]);
        ...
        if (stipple) {
            ...
            list->tex[stipple_tmu] = gen_stipple_tex_coords(list->vert, list->indices, list->mode_inits?list->mode_inits:&tmp, list->vert_stride, list->mode_inits?list->mode_init_len:1, (list->use_glstate)?(list->vert+8+stipple_tmu*4):nullptr);
        }
            ...
            for (int a=0; a<hardext.maxtex; a++) {
                if(glstate->enable.texture[a] || (stipple && a==stipple_tmu)) {
                    ...
                    if ((glstate->enable.texgen_s[a] || glstate->enable.texgen_t[a] || glstate->enable.texgen_r[a]  || glstate->enable.texgen_q[a])) {
                        TEXTURE(a);
                        RS(a, list->len);
                        gen_tex_coords(list->vert, list->normal, &glstate->texgened[a], list->len, &needclean[a], a, (list->ilen<list->len)?indices:nullptr, (list->ilen<list->len)?list->ilen:0);
                    } else if ((list->tex[a]==nullptr) && !(list->mode==GL_POINT && glstate->texture.pscoordreplace[a])) {
                        RS(a, list->len);
                        gen_tex_coords(list->vert, list->normal, &glstate->texgened[a], list->len, &needclean[a], a, (list->ilen<list->len)?indices:nullptr, (list->ilen<list->len)?list->ilen:0);
                    }
            ...
            if (glstate->render_mode == GL_SELECT) {
                ...
                if (glstate->polygon_mode == GL_LINE && list->mode_init>=GL_TRIANGLES) {
                    ...
                    if(list->instanceCount==1)
                        gles_glDrawElements(mode, list->ilen, GL_UNSIGNED_SHORT, vbo_indices?nullptr:indices);
                    else {
            ...
            if (glstate->render_mode == GL_SELECT) {	
                ...
                if ((glstate->polygon_mode == GL_LINE) && (list->mode_init>=GL_TRIANGLES)) {
                    if(!list->ind_lines) {
                        ...
                        int k = fill_lineIndices(list->mode_inits?list->mode_inits:&tmp, list->mode_inits?list->mode_init_len:1, list->mode, nullptr, list->ind_lines);
                        list->ind_line = k;
                    }
                    bindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);
					gles_glDrawElements(mode, list->ind_line, GL_UNSIGNED_SHORT, list->ind_lines);
