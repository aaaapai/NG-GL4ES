...
char* ConvertShader(const char* pEntry, int isVertex, shaderconv_need_t *need, int forwardPort)
{
  ...
  int fpeShader = (strstr(pEntry, fpeshader_signature)!=nullptr)?1:0;
  ...
  if(versionString && (strcmp(versionString, "120")==0 || strcmp(versionString, "110")==0 || strstr(versionString, "150") != nullptr))
...
/*  if(strstr(Tmp, "halfVector"))
  ...
  {
    char *p = Tmp;
    while((p=strstr(p, "centroid"))!=nullptr)
    {
