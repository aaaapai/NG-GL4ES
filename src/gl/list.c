...
/*    if ((a->indices==nullptr) != (b->indices==nullptr))
    ...
    if (a->polygon_mode != b->polygon_mode)
        return false;
    if ((a->vert==nullptr) != (b->vert==nullptr))
        return false;
    if ((a->normal==nullptr) != (b->normal==nullptr))
        return false;
    if ((a->color==nullptr) != (b->color==nullptr))
        return false;
    if ((a->secondary==nullptr) != (b->secondary==nullptr))
        return false;
    if ((a->fogcoord==nullptr) != (b->fogcoord==nullptr))
        return false;
    // check the textures
    if(a->maxtex!=b->maxtex)
        return false;
    for (int i=0; i<a->maxtex; i++)
        if ((a->tex[i]==nullptr) != (b->tex[i]==nullptr))
            return false;
    ...
    if(a->post_color && b->color==nullptr)
        return false;
    // same for post_normal
    if(a->post_normal && b->normal==nullptr)
        return false;
...
void renderlist_quads2triangles(renderlist_t *a) {
    ...
    if(a->use_glstate) {
        if(ind) {//need to copy first...
            ...
            a->shared_indices = nullptr;   // should not be needed
        }
    ...
    if (ind) {
        ...
        a->shared_indices = nullptr; // unshared list
    }
    a->mode = GL_TRIANGLES;
}
#undef vind
...
renderlist_t *extend_renderlist(renderlist_t *list) {
    if ((list->prev!=nullptr) && ispurerender_renderlist(list) && islistscompatible_renderlist(list->prev, list)) {
        // append list!
...
void adjust_renderlist(renderlist_t *list) {
    ...
    for (int a=0; a<list->maxtex; a++) {
	    ...
	    gltexture_t *bound = (itarget>=0)?glstate->texture.bound[a][itarget]:nullptr;
...
renderlist_t* end_renderlist(renderlist_t *list) {
    ...
    switch (list->mode) {
        case GL_QUADS:
			if (((list->indices) && (list->ilen==4)) || ((list->indices==nullptr) && (list->len==4))) {
