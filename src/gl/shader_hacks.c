...
static const hack_t gl4es_hacks[] = {
    ...
    // 1
    {"float edgeGlow = step ( 0.2 , pow ( clamp ( ( dot ( vec2 ( 1 * sign ( v_texcoord3 . z ) , 1 ) , normalize ( "
     "quadCoord . xy - 0.5 ) ) - 0.4 + depth * 2.0 ) , 0.0  , 1.0  ) , 25 ) ) ;",
     1,
     {"float edgeGlow = step ( 0.2 , pow ( clamp ( ( dot ( vec2 ( 1.0 * sign ( v_texcoord3 . z ) , 1.0 ) , normalize ( "
      "quadCoord . xy - 0.5 ) ) - 0.4 + depth * 2.0 ) , 0.0  , 1.0  ) , 25.0 ) ) ;"}},
    // 2
    {"float litfire = max ( dot ( normalize ( drops1 . rgb ) , normalize ( vec3 ( - 1 , 0 , pow ( max ( 1.0 - ocoord . "
     "x , 0.0 ) , 9 ) ) ) ) , 0 ) ;",
     1,
     {"float litfire = max ( dot ( normalize ( drops1 . rgb ) , normalize ( vec3 ( - 1.0 , 0.0 , pow ( max ( 1.0 - "
      "ocoord . x , 0.0 ) , 9.0 ) ) ) ) , 0.0 ) ;"}},
    // 3
    {"if ( ( normalizedDepth ) < 0.0  ) discard ; ;\nif ( depth < 0 )",
     1,
     {"if ( ( normalizedDepth ) < 0.0  ) discard ; ;\nif ( depth < 0.0 )"}},
    // 4
    {"gl_FragColor . rgba += glowHit ;\nif ( depth < 0 )", 1, {"gl_FragColor . rgba += glowHit ;\nif ( depth < 0.0 )"}},
    // 5
    {"gl_FragColor . a *= pow ( clamp ( ( depth + 1 ) , 0.0  , 1.0  ) , 70 ) ;",
     1,
     {"gl_FragColor . a *= pow ( clamp ( ( depth + 1.0 ) , 0.0  , 1.0  ) , 70.0 ) ;"}},
    // 6
    {"if ( floor ( in_texcoord0 . y ) != 0 )", 1, {"if ( floor ( in_texcoord0 . y ) != 0.0 )"}},
    // 7
    {"if ( in_position0 . y < 0 )", 1, {"if ( in_position0 . y < 0.0 )"}},
    // 8
    {"if ( in_position0 . x < 0 )", 1, {"if ( in_position0 . x < 0.0 )"}},
    // 9
    {"branchB . y = 0 ;", 1, {"branchB . y = 0.0 ;"}},
    // 10
    {"branchB . x = 0 ;", 1, {"branchB . x = 0.0 ;"}},
    ...
    // 3
    {"float lightVal = 0;", 1, {"float lightVal = 0.0;"}},
    // 4
    {"       if(dist1 > 0)\n"
     "       {\n"
     "			if(dist1 > 1)\n"
     ...
     {"       if(dist1 > 0.0)\n"
      ...
      "				dist1 = 1.0;\n"}},
    // 5
    {"lightVal += (1-dist1) * light1Luminosity;", 1, {"lightVal += (1.0-dist1) * light1Luminosity;"}},
    // 6
    {"lightVal += (1-dist1) * light2Luminosity;", 1, {"lightVal += (1.0-dist1) * light2Luminosity;"}},
    // 7
    {"lightVal += (1-dist1) * light3Luminosity;", 1, {"lightVal += (1.0-dist1) * light3Luminosity;"}},
    // 8
    {"if(lightVal > 1)\n"
     "			lightVal = 1;",
     1,
    ...
    {"    b /= iterations;", 1, {"    b /= float(iterations);"}},
    ...
    // for IcewindDale
    {"uniform highp \tvec2 \t\tuTcScale;", 1, {"uniform mediump vec2 \t\tuTcScale;"}},

    // for OpenMW
    {"uniform bool simpleWater = false;", 1, {"uniform bool simpleWater;"}},

    // for Lethal League
    {"uniform vec4 Color = vec4(1.0, 1.0, 1.0, 1.0);", 1, {"uniform vec4 Color;"}},

    // for ioQuake3
    {"float c[5] = float[5](1.0, 0.9238795325, 0.7071067812, 0.3826834324, 0.0);",
     1,
     {"float c[5]; c[0]=1.0; c[1]=0.9238795325; c[2]=0.7071067812; c[3]=0.3826834324; c[4]=0.0;"}},

    {"float c[7] = float[7](1.0, 0.9659258263, 0.8660254038, 0.7071067812, 0.5, 0.2588190451, 0.0);",
     1,
     {"float c[7]; c[0]=1.0; c[1]=0.9659258263; c[2]=0.8660254038; c[3]=0.7071067812; c[4]=0.5; c[5]=0.2588190451; "
      "c[6]=0.0;"}},

    {"float scale = 2.0 / r_shadowMapSize;", 1, {"float scale = 2.0 / float(r_shadowMapSize);"}},
    ...
    {"float result = 0;", 1, {"float result = 0.0;"}},
    ...
    {"vec2 offset = direction * j;", 1, {"vec2 offset = direction * float(j);"}},
...
static const char* gl4es_sign_2_main = "void main()\n"
                                       "{\n";
...
static char* ShaderHacks_2_1(char* shader, char* Tmp, int* tmpsize, int i) {
    ...
    if (Tmp == shader) {
        Tmp = (char*)je_malloc(*tmpsize);
        strcpy(Tmp, shader);
        m = strstr(Tmp, gl4es_sign_2_main);
    } // hacking!
