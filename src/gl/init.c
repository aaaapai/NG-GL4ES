...
#define SET_CONFIG_STRING(name) globals4es.name=config_get_string(#name)
    ...
    // pipeline stuff
    env(LIBGL_VGPU_DUMP, globals4es.vgpu_dump, "Dump the content of shader conversion");
    env(LIBGL_VGPU_FORCE, globals4es.vgpu_force_conv, "Force pipeline to convert every shader")
    ...
    if(globals4es.vgpu_precision != 0){
        SHUT_LOGD("default precision overridden ! (%i)", globals4es.vgpu_precision);
    }
