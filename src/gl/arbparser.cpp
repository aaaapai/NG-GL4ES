...
int resolveAttrib(sCurStatus_NewVar *newVar, int vertex) {
	...
	if (!tok) {
		ARBCONV_DBG_RE("Failed to get attrib: (tok nullptr)\n")
		return 1;
	} else if (vertex && !strcmp(tok, "vertex")) {
		free(tok);
		tok = popFIFO((sArray*)newVar);
		if (!tok) {
			ARBCONV_DBG_RE("Failed to get attrib: vertex(tok nullptr)\n")
			return 1;
			...
			tok = popFIFO((sArray*)newVar);
			if (!tok) {
				ARBCONV_DBG_RE("Failed to get attrib: vertex.attrib(tok nullptr)\n")
				return 1;
		...
		if (!tok) {
			ARBCONV_DBG_RE("Failed to get attrib: fragment(tok nullptr)\n")
			return 1;
		} else if (!strcmp(tok, "color")) {
			free(tok);
...
int resolveOutput(sCurStatus_NewVar *newVar, int vertex, struct sSpecialCases *specialCases) {
	...
	if (!tok) {
		ARBCONV_DBG_RE("Failed to get output: (tok nullptr)\n")
		return 1;
		...
		if (!tok) {
			ARBCONV_DBG_RE("Failed to get output: result(tok nullptr)\n")
			return 1;
		} else if (!strcmp(tok, "position")) {
...
char **resolveParam(sCurStatus_NewVar *newVar, int vertex, int type) {
	...
	char *matrixNameMallocd = nullptr;
	size_t mtxNameLen;
	int start = 0;
	int end = 3;
	int isMatrix = 0;
	...
	char *tok = popFIFO((sArray*)newVar);
	if (!tok) {
		ARBCONV_DBG_RE("Failed to get param: (tok nullptr)\n")
		return nullptr;
		...
		if (!tok) {
			ARBCONV_DBG_RE("Failed to get param: state.(tok nullptr)\n")
			return nullptr;
			...
			tok = popFIFO((sArray*)newVar);
			if (!tok) {
				ARBCONV_DBG_RE("Failed to get param: state.material(tok nullptr)\n")
				return nullptr;
			...
			if (!tok) {
				ARBCONV_DBG_RE("Failed to get param: [%s].(tok nullptr)\n", matrixName)
				return nullptr;
			...
			if (newVar->strParts[0][0] == '[') {
				...
				if ((sln[0] >= '0') && (sln[0] <= '9')) {
					...
					tok = popFIFO((sArray*)newVar);
					if (!tok) {
						ARBCONV_DBG_RE("Failed to get param: state.light[%s](tok nullptr)\n", sln)
						return nullptr;
					} else if (!strcmp(tok, "ambient")) {
						free(tok);
						mtxNameLen = 7;
						...
						tok = popFIFO((sArray*)newVar);
						if (!tok) {
							ARBCONV_DBG_RE("Failed to get param: state.light[%s].spot(tok nullptr)\n", sln)
							return nullptr;
			...
			tok = popFIFO((sArray*)newVar);
			if (!tok) {
				ARBCONV_DBG_RE("Failed to get param: state.lightmodel(tok nullptr)\n")
				return nullptr;
			} else if (!strcmp(tok, "ambient")) {
				free(tok);
				char **r = (char**)calloc(2, sizeof(char*));
				...
				tok = popFIFO((sArray*)newVar);
				if (!tok) {
					ARBCONV_DBG_RE("Failed to get param: state.lightmodel.front(tok nullptr)\n")
					return nullptr;
				} else if (!strcmp(tok, "scenecolor")) {
					free(tok);
					char **r = (char**)calloc(2, sizeof(char*));
				...
				tok = popFIFO((sArray*)newVar);
				if (!tok) {
					ARBCONV_DBG_RE("Failed to get param: state.lightmodel.back(tok nullptr)\n")
					return nullptr;
				} else if (!strcmp(tok, "scenecolor")) {
					free(tok);
					char **r = (char**)calloc(2, sizeof(char*));
			...
			if (newVar->strParts[0][0] == '[') {
				...
				if ((sln[0] >= '0') && (sln[0] <= '9')) {
					...
					if (!tok) {
						ARBCONV_DBG_RE("Failed to get param: [%s][%s](tok nullptr)\n", matrixName, sln)
						return nullptr;
					} else if (!strcmp(tok, "ambient")) {
						free(tok);
						propLen = 7;
			...
			if (!tok) {
				ARBCONV_DBG_RE("Failed to get param: state.matrix(tok nullptr)\n")
				return nullptr;
		...
		if (!tok) {
			ARBCONV_DBG_RE("Failed to get param: program(tok nullptr)\n")
			return nullptr;
			...
			if (!tok) {
				ARBCONV_DBG_RE("Failed to get param: program.env(tok nullptr)\n")
				return nullptr;
			} else if (tok[0] == '[') {
				free(tok);
				tok = popFIFO((sArray*)newVar);
			...
			if (!tok) {
				ARBCONV_DBG_RE("Failed to get param: program.local(tok nullptr)\n")
				return nullptr;
			} else if (tok[0] == '[') {
				free(tok);
				tok = popFIFO((sArray*)newVar);
		...
		case TOK_IDENTIFIER: {
			...
			if ((inst = STR2INST(tok, &curStatusPtr->curValue.newInst.inst.saturated)) != INST_UNK) {
				...
				for (int i = 0; i < MAX_OPERANDS; ++i) {
					curStatusPtr->curValue.newInst.inst.vars[i].var = nullptr;
					curStatusPtr->curValue.newInst.inst.vars[i].floatArrAddr = nullptr;
				...
				if ((vtype == VARTYPE_ADDRESS) && !vertex) {
					...
					curStatusPtr->curValue.string = nullptr;
				} else {
				...
				curStatusPtr->curValue.newOpt.optName = nullptr;
			} else {
				free(tok);
			...
			case STATE_RBRACE:       // directly to the resolveParam part...
				if (INSTTEX(curStatusPtr->curValue.newInst.inst.type)
				 && (curStatusPtr->curValue.newInst.curArg == 2)) {
					...
					freeArray((sArray*)&curStatusPtr->_fixedNewVar);
					initArray((sArray*)&curStatusPtr->_fixedNewVar);
					curStatusPtr->_fixedNewVar.var = nullptr;
				}
