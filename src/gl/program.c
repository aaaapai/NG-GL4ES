...
void APIENTRY_GL4ES gl4es_glBindFragDataLocation(GLuint program, GLuint colorNumber, const GLchar* name) {
    ...
    if (glprogram->last_frag) {
        ...
        while (regexec(&regex, searchStart, 1, pmatch, 0) == 0) {
            ...
            size_t resultLen = strlen(origin) + 30; // "layout (location = )" + colorNumber + nullptr terminator
...
static void fill_program(program_t *glprogram)
{
    ...
    int tu_idx = 0;
    for (int i=0; i<n; i++) {
        gles_glGetActiveUniform(glprogram->id, i, maxsize, nullptr, &size, &type, name);
    ...
    for (int i=0; i<n; i++) {
        ...
        DBG(if(e2==GL_NO_ERROR))
        {
            gles_glGetActiveAttrib(glprogram->id, i, maxsize, nullptr, &size, &type, name);
            GLint id = gles_glGetAttribLocation(glprogram->id, name);
            if(id!=-1) {
                attribloc_t *glattribloc = nullptr;
...
void APIENTRY_GL4ES gl4es_glLinkProgram(GLuint program) {
    ...
    if (!glprogram->last_vert) {
        ...
        gl4es_glShaderSource(vtx, 1, fpe_VertexShader(&needs, nullptr), nullptr);
    ...
    if (!glprogram->last_frag) {
        ...
        gl4es_glShaderSource(vtx, 1, fpe_FragmentShader(&needs, nullptr), nullptr);
        gl4es_glCompileShader(vtx);
        gl4es_glAttachShader(glprogram->id, vtx);
    }
    int compatible = 1;
    ...
    if (glprogram->last_vert && glprogram->frag_data_changed == 1) {
        ...
        if (gles_glShaderSource && gles_glCompileShader && gles_glAttachShader) {
            gles_glShaderSource(glprogram->last_frag->id, 1,
                                (const GLchar *const *) &glprogram->last_frag->converted, nullptr);
...
void APIENTRY_GL4ES gl4es_glUseProgram(GLuint program) {
    ...
    if(program==0) {
        ...
        glstate->glsl->glprogram=nullptr;
        return;
    }
    CHECK_PROGRAM(void, program)
    noerrorShim();
